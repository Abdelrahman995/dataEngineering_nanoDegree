...
def visualize_missing_values(df):
    ...
    """ visualize giving dataframe with it's missing values by columns
    :param df:
    :return:
    """
    # lets explore missing values per column
    df_nulls = pd.DataFrame(data= df.isnull().sum(), columns=['values'])
    df_nulls = df_nulls.reset_index()
    df_nulls.columns = ['cols', 'values']

    # calculate % missing values
    df_nulls['% NAN values'] = 100*df_nulls['values']/df.shape[0]

    plt.rcdefaults()
    plt.figure(figsize=(10,5))
    ax = sns.barplot(x="cols", y="% NAN values", data=df_nulls)
    ax.set_ylim(0, 100)
...
def visualize_missing_values_spark(df):
    """Visualizing missing values using a spark dataframe
    ...
    nan_count_df['% nan values'] = 100*nan_count_df['values']/total
    
    plt.rcdefaults()
    plt.figure(figsize=(10,5))
    ax = sns.barplot(x="cols", y="% nan values", data=nan_count_df)
    ax.set_ylim(0, 100)
    ax.set_xticklabels(ax.get_xticklabels(), rotation=90)
    plt.show()
...
def clean_immigration_data(df):
    ...
    columns_to_drop = ['occup', 'entdepu','insnum']
...
def clean_spark_immigration_data(df):
    """Cleaning immigration dataframe
    ...
    print(f'Total number of records in dataframe: {total_records:,}')
    ...
    final_total_records = df.count()
    
    print(f'Total records after cleaning: {final_total_records:,}')
...
def clean_spark_temperature_data(df):
    ...
    print('Total records after dropping rows with nan values: {:,}'.format(total_records-total_recs_after_dropping_nas))
...
def aggregate_temperature_data(df):
    """Aggregate on clean temperature data at country level 
...
def clean_spark_demographics_data(df):
    """Cleaning the US demographics dataset
