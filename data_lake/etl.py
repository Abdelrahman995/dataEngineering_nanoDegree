...
def process_song_data(spark, input_data, output_data):
    """
    Loading data from song_data dataset and extract columns
    for songs and artist tables and write the data into parquet format
    files which will be loaded on aws s3.
    params
    ----------------------------
    spark: session
          Takes spark session that have been created
    input_data: path
           Path to the song_data s3 bucket.
    output_data: path
            Path to where the parquet files to write.
    """
    ...
    # writing songs data table to parquet files partitioned by year and artist
    ...
    # write artists data table to parquet files
    artists_table.write.parquet(os.path.join(output_data, 'artists/artists.parquet'), 'overwrite')


def process_log_data(spark, input_data, output_data):
    """
    Loading data from log data dataset and extract columns
    for users and time tables, reading both the log_data and song_data
    datasets and extracts columns for songplays table with the data.
    and writting the data into parquet files which will be loaded on s3.
    params
    ----------------------------
    spark: session
        Takes spark session that have been created
